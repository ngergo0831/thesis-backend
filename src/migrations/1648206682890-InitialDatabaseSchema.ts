...
export class InitialDatabaseSchema1648206682890 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`intake\` (\`id\` varchar(36) NOT NULL, \`calorie\` int NOT NULL, \`fat\` int NOT NULL, \`ch\` int NOT NULL, \`protein\` int NOT NULL, \`userId\` varchar(255) NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`dietId\` varchar(36) NULL, UNIQUE INDEX \`REL_afbeca7a437b41ca54b216ae6b\` (\`dietId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`diet\` (\`id\` varchar(36) NOT NULL, \`period\` enum ('Daily', 'Weekly', 'Monthly', 'Yearly') NULL, \`creatorId\` varchar(255) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`intakeId\` varchar(36) NULL, UNIQUE INDEX \`REL_30d7085f6a2f62262e95223553\` (\`intakeId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`intake\` ADD CONSTRAINT \`FK_afbeca7a437b41ca54b216ae6b8\` FOREIGN KEY (\`dietId\`) REFERENCES \`diet\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`intake\` DROP FOREIGN KEY \`FK_afbeca7a437b41ca54b216ae6b8\``);
